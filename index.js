...
const APP_DOMAIN2 = process.env.APP_DOMAIN2;
const repoPath = process.env.CDN_PATH;
...
app.post('/upload', async (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      await githubApi.put(`/repos/${GITHUB_USER}/${CDN_REPO}/contents/${repoPath}${fileName}`, {
        message: `upload: ${fileName}`, content: fileContent,
      });
      res.json({ url: `${APP_DOMAIN2}/${fileName}` });
    } catch (error) {
...
app.get('/:code', async (req, res) => {
  ...
  try {
    const rawUrl = `https://raw.githubusercontent.com/${GITHUB_USER}/${CDN_REPO}/main/${repoPath}${code}`;
